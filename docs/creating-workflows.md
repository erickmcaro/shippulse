...
This guide covers how to create your own ShipPulse workflow.
...
agents:
  - id: my-agent            # Unique within this workflow
    ...
    model: openai/gpt-5      # Optional: scalar model or structured object (see below)
    workspace:
...
```yaml
# Structured form with fallbacks
model:
...
When structured config is used, ShipPulse preserves it as an object in `openclaw.json`.
...
ShipPulse runs workflow agents as isolated cron jobs in OpenClaw. You can override the
...
If omitted, ShipPulse defaults to 30 minutes per agent session.
...
ShipPulse includes shared agents in `agents/shared/` that you can reuse:
...
```bash
shippulse workflow install my-workflow
```
