...
describe("two-phase-integration", () => {
  ...
  // AC2: Without a work model, polling prompt omits model so OpenClaw resolves defaults.
  describe("defaults without polling config", () => {
    it("omits work model when no workModel specified", () => {
      ...
      assert.ok(!prompt.includes('"default"'), "should not hardcode default model");
    });
