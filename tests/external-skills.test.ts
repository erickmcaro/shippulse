...
describe("External skill installation", () => {
  ...
  it("does not interfere with shippulse-workflows skill (bundled)", async () => {
    const mod = await import("../dist/installer/agent-provision.js");
    const workflowDir = path.join(tmpDir, "workflow");
    // Create bundled shippulse-workflows skill in workflow dir
    const bundledSkillDir = path.join(workflowDir, "skills", "shippulse-workflows");
    await fs.mkdir(bundledSkillDir, { recursive: true });
    await fs.writeFile(path.join(bundledSkillDir, "SKILL.md"), "# ShipPulse Workflows");
      ...
      agents: [
        {
          ...
          workspace: {
            ...
            skills: ["shippulse-workflows", "agent-browser"],
            files: {},
          },
        },
      ],
    ...
    // shippulse-workflows should be installed from bundled source
    const bundledDest = path.join(results[0].workspaceDir, "skills", "shippulse-workflows", "SKILL.md");
    const bundledContent = await fs.readFile(bundledDest, "utf-8");
    assert.ok(bundledContent.includes("ShipPulse Workflows"));
