...
describe("two-phase-cron-setup", () => {
  describe("buildPollingPrompt with work model", () => {
    ...
    it("omits explicit work model when none specified", () => {
      const prompt = buildPollingPrompt("feature-dev", "developer");
      assert.ok(prompt.includes("omit this field"), "should omit explicit model");
      assert.ok(!prompt.includes('"default"'), "should not include literal default model");
    });
  ...
  describe("setupAgentCrons config resolution", () => {
    ...
    it("default work model is resolved by OpenClaw when omitted", async () => {
      ...
      assert.ok(prompt.includes("omit this field"), "work model should be omitted");
      assert.ok(!prompt.includes('"default"'), "should not hardcode default");
    });
