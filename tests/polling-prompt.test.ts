...
describe("buildPollingPrompt", () => {
  ...
  it("omits explicit model when workModel not provided", () => {
    const prompt = buildPollingPrompt("feature-dev", "developer");
    assert.ok(prompt.includes("omit this field"), "should instruct default model resolution");
    assert.ok(!prompt.includes('"default"'), "should not hardcode default model id");
  });
  ...
  it("includes fail-fast spawn error handling instructions", () => {
    const prompt = buildPollingPrompt("feature-dev", "developer");
    assert.ok(prompt.includes("spawn_error"), "should include explicit spawn failure marker");
    assert.ok(prompt.includes("step fail"), "should instruct immediate step fail on spawn errors");
  });
});
