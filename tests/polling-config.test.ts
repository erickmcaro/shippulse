...
const WORKFLOW_WITH_TIMEOUT_OVERRIDES = `
id: test-timeout-overrides
name: Test Timeout Overrides
version: 1
...
describe("polling config", () => {
  ...
  before(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "shippulse-polling-test-"));
  });
  ...
  it("parses agent and step pollingTimeoutSeconds overrides", async () => {
    const dir = path.join(tmpDir, "timeout-overrides");
    await fs.mkdir(dir, { recursive: true });
    await fs.writeFile(path.join(dir, "workflow.yml"), WORKFLOW_WITH_TIMEOUT_OVERRIDES);

    const spec = await loadWorkflowSpec(dir);
    assert.equal(spec.polling?.timeoutSeconds, 120);
    assert.equal(spec.agents[0].pollingTimeoutSeconds, 210);
    assert.equal(spec.steps[0].pollingTimeoutSeconds, 300);
  });
});
