...
describe("peekStep - lightweight work check", () => {
  ...
  // We mock getDb by setting the SHIPPULSE_DB_PATH env var to a temp file.
  ...
  before(async () => {
    ...
    tmpDbPath = path.join(os.tmpdir(), `shippulse-test-peek-${crypto.randomUUID()}.db`);
    originalDbPath = process.env.SHIPPULSE_DB_PATH;
    process.env.SHIPPULSE_DB_PATH = tmpDbPath;
  });

  after(async () => {
    // Restore original DB path
    if (originalDbPath !== undefined) {
      process.env.SHIPPULSE_DB_PATH = originalDbPath;
    } else {
      delete process.env.SHIPPULSE_DB_PATH;
    }
    // Clean up temp file
    const fs = await import("node:fs");
