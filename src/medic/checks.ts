...
const MEDIC_STUCK_WARNING_SECONDS = readPositiveIntEnv(
  ...
  Math.max(60, Math.round(MAX_ROLE_TIMEOUT_MS / 1000)),
);
const MEDIC_STUCK_CRITICAL_SECONDS = readPositiveIntEnv(
  "SHIPPULSE_MEDIC_STUCK_CRITICAL_SECONDS",
  MEDIC_STUCK_WARNING_SECONDS * 2,
);
...
export function checkStuckSteps(): MedicFinding[] {
  ...
  const thresholdMs = MEDIC_STUCK_WARNING_SECONDS * 1000;
  ...
  for (const step of stuck) {
    ...
    const severity: MedicSeverity = ageSeconds >= MEDIC_STUCK_CRITICAL_SECONDS ? "critical" : "warning";
    findings.push({
      check: "stuck_steps",
      severity,
...
export function checkDeadRuns(): MedicFinding[] {
  ...
  for (const run of zombies) {
    ...
    const detail = failed.cnt > 0
      ...
      : "All steps are done but run is still marked as running";
...
export function checkOrphanedCrons(
  ...
  // Extract unique workflow IDs from shippulse cron job names
  const workflowIds = new Set<string>();
  for (const job of cronJobs) {
    const match = job.name.match(/^shippulse\/([^/]+)\//);
  ...
  for (const wfId of workflowIds) {
    ...
    if (active.cnt === 0) {
      const jobCount = cronJobs.filter(j => j.name.startsWith(`shippulse/${wfId}/`)).length;
      findings.push({
