/**
 * Medic — the shippulse health watchdog.
 *
  ...
  switch (finding.action) {
    ...
    case "complete_run": {
      ...
      emitEvent({
        ...
        runId: finding.runId,
        workflowId: run.workflow_id,
        detail: "Medic: zombie run had no active/failed steps and was marked completed",
      });
      try { await teardownWorkflowCronsIfIdle(run.workflow_id); } catch {}
      return true;
    }
...
export async function runMedicCheck(): Promise<MedicCheckResult> {
  ...
  try {
    ...
    if (cronResult.ok && cronResult.jobs) {
      const shippulseCrons = cronResult.jobs.filter(j => j.name.startsWith("shippulse/"));
      findings.push(...checkOrphanedCrons(shippulseCrons));
    }
  } catch {
