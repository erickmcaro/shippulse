...
import { resolveShipPulseCli } from "../installer/paths.js";
import { readOpenClawConfig, writeOpenClawConfig } from "../installer/openclaw-config.js";

const MEDIC_CRON_NAME = "shippulse/medic";
...
\`\`\`
sessions_send(sessionKey: "agent:main:main", message: "🚑 ShipPulse Medic Alert: <summary of critical issues>")
\`\`\`
...
async function ensureMedicAgent(): Promise<void> {
  try {
    ...
    if (agents.some((a: any) => a.id === "shippulse-medic")) return;

    if (!config.agents) config.agents = {};
    if (!config.agents.list) config.agents.list = [];
    config.agents.list.push({
      id: "shippulse-medic",
      name: "ShipPulse Medic",
      model: MEDIC_MODEL,
    });
...
async function removeMedicAgent(): Promise<void> {
  try {
    const { path, config } = await readOpenClawConfig();
    const agents = config.agents?.list ?? [];
    const idx = agents.findIndex((a: any) => a.id === "shippulse-medic");
    if (idx === -1) return;
    agents.splice(idx, 1);
    await writeOpenClawConfig(path, config);
  } catch {
