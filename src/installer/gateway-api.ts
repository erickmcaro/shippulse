...
export function buildCreateCronCliArgs(job: AgentCronJobSpec): string[] {
  ...
  // OpenClaw CLI uses --announce for announcement delivery mode.
  // "none" is the default and should omit any delivery flag.
...
export async function createAgentCronJob(job: AgentCronJobSpec): Promise<{ ok: boolean; error?: string; id?: string }> {
  // --- Try HTTP first ---
  ...
  // --- Fallback to CLI ---
  try {
    const args = buildCreateCronCliArgs(job);
...
async function createAgentCronJobHTTP(job: AgentCronJobSpec): Promise<{ ok: boolean; error?: string; id?: string } | null> {
