...
const BINARY_NAME = "shippulse";

/**
 * Ensure `shippulse` is available on PATH by symlinking into ~/.local/bin.
  ...
  if (existsSync(linkPath)) {
    try {
      ...
      if (!stats.isSymbolicLink()) {
        ...
        console.warn(`  ⚠ ${linkPath} exists and is not a symlink — skipping (remove it manually to let shippulse manage it)`);
        return;
      }
