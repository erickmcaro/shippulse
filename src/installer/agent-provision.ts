 ...
 * Skips bundled skills like "shippulse-workflows" which are handled separately.
 */
async function installExternalSkills(workflow: WorkflowSpec): Promise<void> {
  const bundledSkills = new Set(["shippulse-workflows"]);
  ...
  for (const agent of workflow.agents) {
    ...
    for (const skillName of externalSkills) {
      ...
      if (!source) {
        ...
        console.warn(`[shippulse] Skill "${skillName}" not found for agent "${agent.id}", skipping`);
        continue;
      }
...
async function installWorkflowSkill(workflow: WorkflowSpec, workflowDir: string) {
  const skillSource = path.join(workflowDir, "skills", "shippulse-workflows");
  try {
  ...
  for (const agent of workflow.agents) {
    if (!agent.workspace.skills?.includes("shippulse-workflows")) {
      continue;
    }
    const workspaceDir = resolveWorkspaceDir({ workflowId: workflow.id, agent });
    const targetDir = path.join(workspaceDir, "skills");
    await ensureDir(targetDir);
    const destination = path.join(targetDir, "shippulse-workflows");
    await fs.rm(destination, { recursive: true, force: true });
