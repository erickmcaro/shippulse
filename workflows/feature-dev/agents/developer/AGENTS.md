...
6. Append to `progress.txt` (see format below) using shell redirection (`cat <<'EOF' >> progress.txt`)
...
Important: avoid repeated `edit` find/replace loops on progress files. Prefer append operations; if top-of-file changes are required, rewrite the file once.
