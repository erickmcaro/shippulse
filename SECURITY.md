...
ShipPulse workflows run AI agents on your machine. That's powerful — and it means security matters.

## How we keep things safe

### Curated repository only

ShipPulse only installs workflows from this official repository (`snarktank/shippulse`). There is no mechanism to install workflows from arbitrary URLs, third-party repos, or remote sources. If it's not in this repo, it doesn't run.
...
If you find a security issue in ShipPulse, please report it responsibly:
